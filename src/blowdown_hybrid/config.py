...
from src.cea_hybrid.config import ensure_finite
...
from .thermo import validate_n2o_temperature_k
...
def build_config(raw=None):
    ...
    config = {
        ...
        "injector": {
            ...
            "hole_diameter_m": float(merged["injector"].get("hole_diameter_m", PROJECT_DEFAULT_INJECTOR_HOLE_DIAMETER_M)),
...
def validate_config(config):
    ...
    for name, value in [
        ...
        ("injector.hole_diameter_m", config["injector"]["hole_diameter_m"]),
    ...
    if config["injector"]["hole_diameter_m"] <= 0.0:
        raise ValueError("Injector hole diameter must be positive.")
