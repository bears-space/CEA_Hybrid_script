"""Reduced-order chamber-region thermal evaluation."""
...
def evaluate_cylindrical_region(
    *,
    region_name: str,
    ...
    gas_temperature_scale: float,
) -> ChamberThermalResult:
    """Evaluate a cylindrical chamber-like region with the shared wall model."""

    if region_length_m <= 0.0:
        raise ValueError(f"{region_name} length must be positive for thermal evaluation.")
    surface_area_m2 = math.pi * float(inner_diameter_m) * float(region_length_m)
    gas_temperature_history_k = [
        effective_gas_temperature_k(region_name, value) * float(gas_temperature_scale)
    ...
    if wall_result["peak_biot_number"] > 0.1:
        warnings.append(
            f"{region_name}: lumped wall assumption is outside the usual Biot <= 0.1 range; treat temperatures as first-pass only."
        )
    if peak_inner > allowable_k:
        warnings.append(f"{region_name}: peak inner-wall temperature exceeds the selected allowable temperature basis.")
    region = RegionThermalResult(
        region_name=region_name,
        material_name=material.material_name,
        selected_wall_thickness_m=float(wall_thickness_m),
        ...
        model_assumptions=[
            "Bartz-like gas-side HTC placeholder.",
            "Two-node wall model per unit surface area.",
            "No combustion-side radiation model beyond optional ambient-side radiation rejection.",
        ],
        warnings=warnings,
    )
    return ChamberThermalResult(
        region=region,
        inner_surface_area_m2=surface_area_m2,
        gas_reference_temp_k=max(gas_temperature_history_k),
    )
