"""Reduced-order throat and nozzle thermal evaluation."""
...
def evaluate_nozzle_region(
    *,
    ...
    """Evaluate a throat or nozzle region with the shared wall model."""

    gas_temperature_history_k = [
        effective_gas_temperature_k(region_name, value, area_ratio=area_ratio)
    ...
    if region_name == "throat" and wall_thickness_m < 0.0015:
        warnings.append("throat: selected bare-wall thickness is very low for a rocket-throat placeholder.")
    ...
    region = RegionThermalResult(
        ...
        model_assumptions=[
            "Bartz-like gas-side HTC placeholder with region-specific multipliers.",
            "Two-node wall model per unit surface area.",
        ],
        warnings=warnings,
    )
    return NozzleThermalResult(
        region=region,
        axial_length_m=float(axial_length_m),
        characteristic_diameter_m=float(characteristic_diameter_m),
    )
