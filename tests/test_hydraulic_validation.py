...
from src.hydraulic_validation.hydraulic_predictor import injector_delta_p_from_mdot
from src.io_utils import deep_merge
from src.simulation.solver_0d import prepare_runtime_case


class HydraulicValidationTests(unittest.TestCase):
    ...
    def _hydraulic_config(self, config, dataset_path: Path, *, calibration_mode: str = "joint") -> dict:
        override = {
        ...
        return normalize_hydraulic_validation_config(deep_merge(config["hydraulic_validation"], override), config)
    ...
    def _write_synthetic_dataset(
        ...
        *,
        ...
        hydraulic_config = self._hydraulic_config(config, destination)
        context = build_prediction_context(config, hydraulic_config)
        actual_feed = replace(
            context.feed_config,
        ...
        density_kg_m3 = float(hydraulic_config["fluid"]["density_kg_m3"])
    ...
    def test_hydraulic_prediction_matches_synthetic_baseline(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            hydraulic_config = self._hydraulic_config(config, dataset_path)

            payload = run_hydraulic_prediction_workflow(config, hydraulic_config, output_dir)
            ...
            self.assertTrue((output_dir / "hydraulic_predictions.csv").exists())
            self.assertTrue((output_dir / "mdot_parity.svg").exists())

    def test_joint_hydraulic_calibration_recovers_synthetic_multipliers(self):
        config = self._small_config()
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            hydraulic_config = self._hydraulic_config(config, dataset_path, calibration_mode="joint")

            payload = run_hydraulic_calibration_workflow(config, hydraulic_config, output_dir)
    ...
    def test_prepare_runtime_case_applies_saved_hydraulic_package(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            hydraulic_config = self._hydraulic_config(base_config, dataset_path, calibration_mode="joint")

            payload = run_hydraulic_calibration_workflow(base_config, hydraulic_config, output_dir)
            package = payload["calibration_package"]

            calibrated_config = self._small_config()
            calibrated_config["hydraulic_validation"]["hydraulic_source"] = "coldflow_calibrated"
            calibrated_config["hydraulic_validation"]["calibration_package_path"] = str(output_dir / "calibration_package.json")
            calibrated_config["hydraulic_validation"]["allow_missing_calibration_package"] = False
