...
class Ballistics1DSmokeTests(unittest.TestCase):
    def _small_config(self):
        ...
        config["ballistics_1d"]["axial_cell_count"] = 7
        config["ballistics_1d"]["time_step_s"] = 0.02
        config["ballistics_1d"]["max_simulation_time_s"] = 0.4
        config["ballistics_1d"]["record_every_n_steps"] = 1
        config["ballistics_1d"]["station_sample_count"] = 3
        config["ballistics_1d"]["compare_to_0d"] = False
        config["ballistics_1d"]["max_port_growth_fraction_per_step"] = 0.5
        return config

    def test_ballistics_1d_runs_from_frozen_geometry(self):
        config = self._small_config()
        nominal = run_nominal_case(config)
        geometry = freeze_first_pass_geometry(config, None, nominal)

        payload = run_ballistics_case(config, geometry)
        ...
        self.assertEqual(payload["result"]["axial_history"]["port_radius_m"].shape[1], config["ballistics_1d"]["axial_cell_count"])
        self.assertIn("port_diameter_mid_final_mm", payload["metrics"])
        self.assertIn("oxidizer_flux_max_kg_m2_s", payload["metrics"])

    def test_ballistics_1d_can_compare_against_nominal_0d(self):
        config = self._small_config()
        nominal = run_nominal_case(config)
        geometry = freeze_first_pass_geometry(config, None, nominal)

        payload = run_ballistics_case(config, geometry, compare_payload=nominal)
    ...
    def test_ballistics_1d_terminal_snapshot_matches_end_of_run_geometry(self):
        config = self._small_config()
        ...
        config["ballistics_1d"]["time_step_s"] = 0.01
        config["ballistics_1d"]["max_simulation_time_s"] = 0.01
        config["ballistics_1d"]["max_port_growth_fraction_per_step"] = 10.0
