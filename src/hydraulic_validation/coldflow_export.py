"""Hydraulic validation output export, reports, and lightweight plots."""
...
def write_coldflow_outputs(
    ...
    *,
    ...
    """Write the standard hydraulic-validation output bundle."""
    ...
    write_rows_csv(destination / "hydraulic_predictions.csv", combined_predictions)
    write_rows_csv(destination / "hydraulic_residuals.csv", combined_residuals)
    write_json(destination / "hydraulic_dataset.json", dataset.to_dict())
    write_json(destination / "hydraulic_baseline_stats.json", dict(baseline_stats))
    if calibrated_stats is not None:
        write_json(destination / "hydraulic_calibrated_stats.json", dict(calibrated_stats))
    ...
    if calibration_package is not None:
        ...
        if calibration_package.recommended_model_source == "geometry_plus_coldflow":
            override_payload["injector_design"] = {"solver_injector_source": "geometry_backcalculated"}
    ...
    (destination / "hydraulic_validation_summary.txt").write_text(
        "\n".join(
            _summary_lines(
                dataset=dataset,
