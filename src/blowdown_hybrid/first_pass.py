...
def injector_hole_count_from_total_area(
    injector_total_area_m2: float,
    injector_hole_diameter_m: float,
) -> int:
    """Return the integer hole count needed to meet or exceed the required total injector area."""

    if injector_total_area_m2 <= 0.0:
        raise ValueError("injector_total_area_m2 must be positive.")
    if injector_hole_diameter_m <= 0.0:
        raise ValueError("injector_hole_diameter_m must be positive.")
    area_per_hole_m2 = math.pi * (0.5 * injector_hole_diameter_m) ** 2
    return max(1, int(math.ceil(injector_total_area_m2 / area_per_hole_m2)))


def injector_total_area_from_hole_count_and_diameter(
    injector_hole_count: int,
    injector_hole_diameter_m: float,
) -> float:
    """Return the realized injector total area for an integer hole count and fixed hole diameter."""

    if injector_hole_count <= 0:
        raise ValueError("injector_hole_count must be greater than zero.")
    if injector_hole_diameter_m <= 0.0:
        raise ValueError("injector_hole_diameter_m must be positive.")
    return injector_hole_count * math.pi * (0.5 * injector_hole_diameter_m) ** 2
