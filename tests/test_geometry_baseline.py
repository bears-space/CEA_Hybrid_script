...
class GeometryBaselineSmokeTests(unittest.TestCase):
    ...
    def test_freeze_geometry_returns_structured_definition(self):
        ...
        self.assertGreater(geometry.converging_section_length_m or 0.0, 0.0)
        self.assertGreater(geometry.nozzle_length_m or 0.0, 0.0)
        self.assertEqual(geometry.nozzle_contour_style, "conical_blended")
        self.assertGreater(geometry.throat_blend_radius_m or 0.0, 0.0)
        self.assertGreater(geometry.chamber_outer_diameter_excluding_liner_m or 0.0, geometry.chamber_id_m)
    ...
    def test_geometry_export_writes_expected_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue((output_dir / "geometry_definition.json").exists())
            self.assertTrue((output_dir / "geometry_definition.csv").exists())
            self.assertTrue((output_dir / "geometry_main_outputs.json").exists())
            self.assertTrue((output_dir / "geometry_main_outputs.csv").exists())
            self.assertTrue((output_dir / "geometry_summary.txt").exists())

            payload = json.loads((output_dir / "geometry_definition.json").read_text(encoding="utf-8"))
            self.assertIn("lstar_initial_m", payload)
            self.assertIn("geometry_valid", payload)
            self.assertIn("converging_section_length_m", payload)
            self.assertIn("nozzle_length_m", payload)
            self.assertIn("chamber_inner_diameter_excluding_liner_m", payload)
            self.assertEqual(payload["nozzle_contour_style"], "conical_blended")

            main_outputs = json.loads((output_dir / "geometry_main_outputs.json").read_text(encoding="utf-8"))
            labels = {row["label"] for row in main_outputs["outputs"]}
            self.assertIn("Average specific impulse", labels)
            self.assertIn("Average thrust", labels)
            self.assertIn("Burn time", labels)
